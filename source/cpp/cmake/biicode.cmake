...
macro(BII_INCLUDE_BLOCK BLOCK_DIR)
	...
	file(TO_NATIVE_PATH "${CMAKE_HOME_DIRECTORY}/../${BLOCK_DIR}/CMakeLists.txt" _CMAKELISTPATH)

	if(NOT EXISTS ${_CMAKELISTPATH})
		message(FATAL_ERROR "ERROR, MISSING CMakeLists.txt at: ${_CMAKELISTPATH}")
	else()
		ADD_SUBDIRECTORY("${CMAKE_HOME_DIRECTORY}/../${BLOCK_DIR}" "${BII_BLOCK_USER}/${BII_BLOCK_NAME}")
		VALIDATE_BLOCK_CMAKELIST("${CMAKE_HOME_DIRECTORY}/../${BLOCK_DIR}")
	endif()
endmacro()
...
function(BII_GENERATE_EXECUTABLE USER BLOCK FNAME)
	...
	if(COMMAND BII_ADD_EXECUTABLE)
        BII_ADD_EXECUTABLE(${vname} "${aux_src}")
    else()
...
function(BII_GENERATE_LIB USER BLOCK)
    ...
    if(BII_LIB_DEPS)
        TARGET_LINK_LIBRARIES( ${vname} ${TRANSITIVE} ${BII_LIB_DEPS})
    endif()
    if(BII_LIB_INCLUDE_PATHS)
        target_include_directories( ${vname} ${TRANSITIVE} ${BII_LIB_INCLUDE_PATHS})
    endif()
    if(BII_LIB_SYSTEM_HEADERS)
        HANDLE_SYSTEM_DEPS(${vname} ${TRANSITIVE} "BII_LIB_SYSTEM_HEADERS")
    endif()
endfunction()
...
function(HANDLE_SYSTEM_DEPS target_name ACCESS sys_deps)
	if((${BII_IMPLICIT_RULES_ENABLED}) AND NOT (${CMAKE_SYSTEM_NAME} STREQUAL "Arduino"))
