...
from segmentation_filters import create_default_filter_stack, create_min_filter_stack, Filter, NearLineFilter
import numpy
import cv2
...
class RawContourSegmenter( RawSegmenter ):
    PARAMETERS=  RawSegmenter.PARAMETERS + {"block_size":11, "c":10 } # defaults
    def _segment( self, image ):
        self.image= image
        image= cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
        image = cv2.adaptiveThreshold(image, maxValue=255, adaptiveMethod=cv2.ADAPTIVE_THRESH_MEAN_C, thresholdType=cv2.THRESH_BINARY, blockSize=self.block_size, C=self.c)
    ...
    def display(self, display_before=False):
        ...
        key = show_image_and_wait_for_key(copy, "image after segmentation by "+self.__class__.__name__)
        return key
...
class MinContourSegmenter( FullSegmenter ):
    def __init__(self, **args):
        filters= create_min_filter_stack()
        stack = [BlurProcessor(), RawContourSegmenter()] + filters + [SegmentOrderer()]
        FullSegmenter.__init__(self, stack, **args)
        stack[0].add_prehook( create_broadcast( "_input", filters, "image" ) )
